...
const PlatformAdministratorProfile = ({openSnackbar}) => {
    ...
    const updatePlatformAdministratorProfile = async () => {
        try {
            if (ValidationService.validateMiddleName(middleName) && ValidationService.validateEmail(email)) {
                await APIService.updatePlatformAdministrator(accessToken, user.id, middleName, email);
                    ...
                    <Grid className={classes.profileContainer}>
                        {!edit ? (
                            <Grid container direction={"column"} alignItems={"flex-start"} className={classes.root}>
                                ...
                                <Button fullWidth={true} target="_blank" href="http://localhost:8000/admin/">
                            ...
                            <Grid container direction={"column"} className={classes.root}>
                                ...
                                <Button onClick={() => updatePlatformAdministratorProfile()}>Save</Button>
...
export default PlatformAdministratorProfile;